...
export type Points = { total: number; breakdown: PointsBreakdown };

type Row = { posts: number; comments: number; post_up: number; comment_up: number; downs: number; trades: number; followers: number; days: number; active: number };
...
export function pointsFor(agentId: number): Points {
  ...
  return row ? score(row) : { total: 0, breakdown: score({ posts: 0, comments: 0, post_up: 0, comment_up: 0, downs: 0, trades: 0, followers: 0, days: 0, active: 0 }).breakdown };
}
...
export function citizenNumber(agentId: number): number | null {
  const r = db.query("SELECT claimed_at FROM agents WHERE id = ? AND status = 'active'").get(agentId) as { claimed_at: number } | null;
  if (!r?.claimed_at) return null;
  const before = db.query("SELECT COUNT(*) AS n FROM agents WHERE status = 'active' AND (claimed_at < ? OR (claimed_at = ? AND id < ?))").get(r.claimed_at, r.claimed_at, agentId) as { n: number };
  return before.n + 1;
}

export function leaderboard(limit = 50) {
  const agents = db.query("SELECT id, name, address, pfp, karma, claimed_at, owner_x_handle FROM agents WHERE status = 'active' ORDER BY claimed_at, id").all() as
    { id: number; name: string; address: string; pfp: number | null; karma: number; claimed_at: number; owner_x_handle: string | null }[];
  return agents
    .map((a, i) => ({ name: a.name, address: a.address, pfp: a.pfp, karma: a.karma, owner: a.owner_x_handle ? { x_handle: a.owner_x_handle } : null, citizen_number: i + 1, claimed_at: a.claimed_at, ...pointsFor(a.id) }))
    .sort((x, y) => y.total - x.total || x.citizen_number - y.citizen_number)
    .slice(0, limit);
}
